...
const RATE_LIMITS = {
  ...
  ANTHROPIC: {
    DEFAULT: 60 * 1000 / 40, // 40 requests per minute (RPM)
    CHAT: 60 * 1000 / 40,    // 40 RPM
  }
};
...
export const createLimiters = (apiType: 'OPENAI' | 'ANTHROPIC' | 'STRIPE', endpointType?: 'CHAT' | 'IMAGE' | 'AUDIO') => {
  ...
  switch (apiType) {
    case 'OPENAI':
      ...
      break;
    case 'ANTHROPIC':
      rateLimit = endpointType && endpointType in RATE_LIMITS.ANTHROPIC
        ? RATE_LIMITS.ANTHROPIC[endpointType as keyof typeof RATE_LIMITS.ANTHROPIC]
        : RATE_LIMITS.ANTHROPIC.DEFAULT;
      concurrencyLimit = endpointType && endpointType in CONCURRENCY_LIMITS.ANTHROPIC
        ? CONCURRENCY_LIMITS.ANTHROPIC[endpointType as keyof typeof CONCURRENCY_LIMITS.ANTHROPIC]
        : 3;
      break;
    default:
...
// Helper function to extract error messages from Anthropic errors
export const extractAnthropicErrorMessage = (error: any): string => {
  if (!error) return 'Unknown error';
  ...
  if (error.message) {
    if (error.message.includes('rate_limit') || error.message.includes('429')) {
      ...
      message = 'Invalid API key. Please check your API key configuration.';
    } else if (error.message.includes('content_policy') || error.message.includes('violates')) {
      message = 'Your request was flagged for content policy violation. Please modify and try again.';
    } else if (error.message.includes('context_length') || error.message.includes('too_long')) {
      message = 'The conversation is too long for the model. Please start a new conversation.';
